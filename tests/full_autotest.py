...
def l1_audio_model() -> None:
    ...
    record("audio.track.4CH 流不得塌缩成不可再分的 1 个对象",
           ...
           and tracks[0].source_ids == ["s2c0", "s2c1", "s2c2", "s2c3"],
           f"{tracks[0].track_id}")
    ...
    record("audio.json 未来字段/未知 enum 不崩溃",
           ...
           == 99)
    record("audio.json.model_version 常量", AUDIO_MODEL_VERSION == 1)
...
def l3_audio_probe() -> None:
    ...
    if ok4:
        ...
        record("l3.audio.4CH 单通道选择可用 (不塌缩)",
               ...
               == [f"s{trk4[0].source_stream_index}c2"]
    ...
    if okm:
        ...
        record("l3.audio.同步后身份仍可追溯原始 stream/channel",
               ...
               == [[f"s{i}c0"] for i in probe_m.stream_indices])
    else:
