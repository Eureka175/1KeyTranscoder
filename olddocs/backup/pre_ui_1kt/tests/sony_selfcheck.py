...
解析。所有条目 + PASS/FAIL 写 log 入盘 (JSON + 可读文本), 供内部测试
审计。

定位: 内部测试使用, 验证主程序交付物的元数据保留是否正确完整。
用法:
...
from preservation.gyroflow import _export, _facts  # noqa: E402
from preservation.sony import ParsedFile  # noqa: E402

PASS = "PASS"
FAIL = "FAIL"
SKIP = "SKIP"

_FIELDS = ("version", "minor_version", "compatible_brands")
...
def _payload_facts(
    ...
    with raw.open("rb") as f:
        head = f.read(32)
        f.seek(max(0, raw.stat().st_size - 32))
        tail = f.read(32)
    try:
        raw.unlink()
    except OSError:
...
def detailed_compare(
    *,
    ...
    def track_map(p: ParsedFile) -> dict[str, dict[str, Any]]:
    ...
    ov = video_facts(out)
    if sv and ov:
        for key in ("timescale", "stts", "stsd"):
    ...
    if s_v and o_v:
        ...
        note("video.ffprobe.codec", s_v.get("codec_name"), o_v.get("codec_name"))

    # audio per-track
    s_audio = sorted(
        (t for ts in src_tracks.get("soun", []) for t in [ts]),
        key=lambda t: t["track_id"],
    )
    o_audio = sorted(
        (t for ts in out_tracks.get("soun", []) for t in [ts]),
        key=lambda t: t["track_id"],
    )
    ...
    if s_d is not None or o_d is not None:
        ...
        note("rtmd.tag.timecode", s_tags.get("timecode"), o_tags.get("timecode"))
        note(
    ...
    if gyroflow is not None:
        try:
            g_src = _facts(_export(gyroflow, original, scratch / "g_orig.json"))
            g_out = _facts(_export(gyroflow, final, scratch / "g_final.json"))
            mismatches = {
...
def main(argv: list[str]) -> int:
    if len(argv) != 3:
        print(
            "usage: python tests/sony_selfcheck.py "
    ...
    print(
        f"selfcheck: {report['overall']} {report['summary']} "
        f"-> {log_dir}"
    )
