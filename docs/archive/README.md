# docs/archive/ — 归档区与状态索引

> **归档区是"已完成/已封存"文档的存放处，不是垃圾场。** 这里的东西**结论仍然
> 有效**、可以引用、可以追溯，只是**不再随代码演进更新**。判断一份文档该不该
> 进来，只看一条：**它的结论是否已经被后续工作收口、且不再有活跃维护者？**
>
> 活跃文档请放回 `docs/design/`、`docs/evaluation/` 等分类目录。
> 历史**代码**快照不放这里，在根目录 `olddocs/`。
