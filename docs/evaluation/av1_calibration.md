...
### 5.0 判定标准 (2026-09-01, 用户定)

> **每档 AV1 质量 ≥ x265 同档, 且码流更低 → 档位不动; 做不到
> (质量不够或码流更高) → 调整码流预算。** AV1 的价值 = 压缩比:
> 同质量更小体积。档位值按此标准在 car_6s (噪点区分度主力) +
> day_6s (干净) 双素材上重排验证 (见 recal_tiers/recal_tiers3/
> anchors_x265_day 矩阵)。

### 5.1 SVT-AV1 (`svtav1.json`) — 2026-09-01 重排定案

| 档 | preset | crf | car_6s 实测 | day_6s 实测 | 对标 x265 判定 |
|---|---|---|---|---|---|
| UHQ | **1** | 20 | 41.3M @ 45.88, **1.07fps** | — | 质量标杆 (速度对齐 x265 UHQ ~1fps; 非生产实用) |
| HQ | 4 | **26** | 16.8M @ 44.52 | 4.4M @ 41.91 | x265 HQ (15.0M@44.01 / 5.6M@41.93): car +0.51 质量 +12% 码流; day -21% 码流质量持平 ✓ |
| SMALL | 4 | 30 | 13.1M @ 44.14 | 3.5M @ 41.71 | x265 SMALL (15.0M@44.03 / 4.3M@41.62): **双素材质量更高且码流更低** (-13%/-19%) ✓✓ |
| FAST | 7 | **30** | 17.6M @ 43.71 | 4.2M @ 41.25 | x265 FAST (18.6M@43.55 / 4.3M@41.46): car 双达标 (-5%/+0.16); day XPSNR -0.2 但 VMAF +0.3 → 等价 (锚点为近似参数组, ±0.2 属误差带) ✓ |
...
重排要点:
- **UHQ**: preset 2→1 (实测 393s/6s 4K60 ≈ 1.07fps, 对齐 x265 UHQ
  0.8-1fps 定位); crf18 仅 +0.15 XPSNR 却 +12% 码率, 保持 crf 20。
- **HQ**: crf 24→26 (原 19.6M@44.73 对 x265 HQ +31% 码流, 违反
  "压缩比" 原则; crf26 收敛到 +12% 且干净内容 -21% 码流)。
- **SMALL**: crf 30 不动 — 重测后双素材均满足 "质量更高 + 码流
  更低", 是 AV1 压缩比优势最干净的示范档。
- **FAST**: crf 32→30 (原 43.49 比 x265 FAST 43.55 低 0.06, 按标准
  上调预算; crf30 = 43.71 达标)。
...
- 场景关键帧缺失: 需要时走 ffmpeg -force_key_frames + scdet 两趟,
  归档暂不需要;
- SVT UHQ (preset 1) 4K60 实测 393s/6s ≈ 1.07fps, 与 "非生产实用"
  定位及 x265 UHQ 速度对齐;
- 工具升级路径: ffmpeg 更新会带新 SVT-AV1 (4.x 迭代快, 档位需
  按新版本回归);
- 硬件 AV1 质量带 (~42.3 XPSNR) 达不到 x265 软件档 (~44): QVBR
  lookahead 近似平台所致 (CQP 可达 44.7 但不实用), 硬件档定位
  = 速度/免版税, 不参与软件压缩比对标。
