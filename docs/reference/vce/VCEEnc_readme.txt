...
2026.08.09 (9.12)
[VCEEncC]
- 古いGPUと新しいGPUが混在する環境でのドライバロードを改善。
- avsw入力で入力途中の解像度変更に追従するように。
- --adapt-resolutionで可変解像度入力の拡大上限を指定可能に。
- PMT変更追従機能を追加。
- 音声・字幕の言語除外指定に対応。
- --vpp-kfm mode=24の長尺処理でGPUメモリが増加する問題を修正。
- --vpp-kfmのメモリ保持を改善。
- インタレ保持でのリサイズなど、フィールド単位のコピー処理を修正。
- avhw入力で入力途中の解像度変更を検出して明示エラーとするように。
...
- Dolby Visionプロファイル適用時に、プロファイル要件に応じて--repeat-headers、--aud、--enforce-hrdを実際に有効化するように修正。Dolby Vision RPU出力向けにビットレート/HRDの注意をドキュメントに追記。
- Linuxでもファイル分割並列エンコード(--parallel)を利用可能に。
- Ubuntu 24.04向けAMF回避策をインストール手順に追加。
- ビルド／インストール関連ドキュメントを更新。

2025.03.15 (9.04)
[VCEEncC]
- dtsxがコピーできない問題への対策。
- check-hwでAMF初期化失敗を表示するように。
- E-AC3のエンコードが終了処理でエラーとなる問題を修正。
- mux時にencoding_toolsにエンコーダのオプションを記入するオプションを追加。(--muxer-add-cmd)
- --audio-bitrateを拡張し、チャンネルレイアウトごとにビットレートを指定できるように。
- 音声のビットレートについてのログ表示を修正。
[Linux]
- Linux通常エンコードのGPU選択とOpenCL追従を修正。
- Linux向けAMF/Vulkan初期化と機能検出を修正。
- linuxでのvapoursynthの読み込みを修正。
- LinuxのマルチGPU環境で、SIGPIPE(141)でエラー終了する可能性があったのを修正。
- Linuxでmesonを使用したビルドに移行。
- Linuxでのeventの処理の見直し。
[VCEEnc.auo]
- 中国語翻訳の更新。(#137)
...
[VCEEnc.auo]
- 簡易アンインストーラを追加。
  プラグインフォルダの VCEEnc_uninstall.bat をダブルクリックで実行すると、下記確認画面で出ます。
...
- --presetでH.264/HEVCでもslowerを指定できるように。
...
- --qvbrの指定が適切に設定されない問題を修正。
- Linux環境で、Vulkanで認識されるGPUが0番目でないとエラー終了する問題を回避。
- Ubuntu 24.04向けビルドを追加。

2025.03.04 (8.32)
- AV1エンコードで--dolby-vision-rpu使用時の処理を改善。

2025.03.01 (8.31)
- 8.29からavswで一部のコーデックがデコードできなくなっていた問題を修正。
...
- HEVCのqvbr-qualityが正常に反映されない問題を修正。
