...
| 性能 | — | 10min/4ch/48k 全流程（解码+估计+修正+回编码）实测 ~31s；算法 benchmark（无重编码）Peak RSS **153–288 MB** / **45.3–52.7 s**（v0.6.1 五次实测，目标 ≤512MB / <60s） |
...
**结果**：600 s/4ch/48k 峰值 RSS 阶段探针 719.0 MB → 288.5 MB，正式算法
benchmark **153–288 MB**（v0.6.0 同场景 605–713 MB），
