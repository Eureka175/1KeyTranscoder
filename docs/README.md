...
| `e2e-benchmark.md` | **S9 端到端基准（已执行，含 patched `--avhw`）**：软解 vs **patched** rigaya `--avhw` vs stock `--avhw` vs FFmpeg NVDEC→传输→编码；Sony/DJI × 60s × 1路/2路/4路；含逐帧完整性证明（SHA-256 字节相同）、精确 CPU 计量与 validity gate；§0.1 = 已观测/推断未证/未测三者分离，§8.1 = 集成 session 行动指南 |
| `research-conclusion.md` | **★四条 research 分支的最终交叉结论**：已验证状态表、三个独立根因（NVEncC 输出级过滤 / QSV 准入过滤 / `FramePosList` 元数据表）、工程建议与结论边界、集成前置条件、开放事项。**进入 integration 前先读这一份。** |
...
> 该文件已随 `research/hwdecode-e2e-benchmark` 分支收口，新增 §0.1（**已观测 /
> 推断未证 / 未测**三者分离）与 §8.1（集成 session 行动指南）。
>
...
>   （**注意：崩溃的机制属推断，未由 VRAM telemetry 直接证明**）。
>
> **一句话结论：硬件解码的主要收益是 CPU 余量，而不是单任务速度提升。**
>
> 建议：**保留软解为默认 + 硬解可选（patched `--avhw`）**，其价值是 CPU 余量而非速度；
> FFmpeg `-hwaccel` 保留为正确性/参考路径，**不作生产主路径**
> （raw pipe 会损失约 80% 的 CPU 收益）。
> 四条 research 分支的最终交叉结论见 `docs/hardware-decode/research-conclusion.md`。
